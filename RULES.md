...
5. **No comments in code: only docs.** Code carries no comments. Whatever a comment would say goes in the documentation — a docstring where the language has them, otherwise `AGENTS.md` or a README — or nowhere, because the code already says it. A shebang, a licence header or a linter directive is not a comment in this sense.
